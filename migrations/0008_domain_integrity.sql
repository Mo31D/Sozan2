...
-- Keep trigger bodies deliberately free of CASE ... END; constructs because
-- Cloudflare D1's remote migration statement splitter can misread inner END;
-- tokens as the end of the trigger. SELECT RAISE ... WHERE preserves the same
-- invariant while remaining safe for Wrangler/D1 remote migrations.
-- ---------------------------------------------------------------------------
...
BEGIN
  SELECT RAISE(ABORT, 'RECEIPT_NOT_FOUND')
  WHERE NOT EXISTS (
