...
def main() -> None:
    ...
    # Reproduce the important production ordering: the reviewed workspace exists
    # when 0007 runs, then the general integrity schema and targeted repair run.
    ...
    # Verify a genuinely changed/user-owned financial configuration is not
    # mistaken for the 0007 synthetic default by the targeted cleanup.
    apply(db, "0007_correct_tutoring_data_2026_09_17.sql")
    ...
    # Storage-level money conservation must hold independently from application
    # code and retry logic.
    db.execute(
