...
export function WeekView({ data, onEdit, onOpenStudent }: { data: SimpleWorkspaceData; onEdit: (sessionId: string) => void; onOpenStudent: (studentId: string) => void }) {
      ...
      {Array.from({ length: 7 }, (_, index) => addDays(start, index)).map((date, index) => {
        ...
        return (
          <section className={`day-block ${index === 0 ? 'today-day' : ''}`} key={date}>
            ...
            {rows.length
              ? rows.map((entry) => {
                  const linkedStudents = entry.session.studentIds
                    .map((id) => data.students.find((student) => student.id === id))
                    .filter((student): student is NonNullable<typeof student> => Boolean(student));
                  return (
                    <div className="schedule-entry-with-students" key={`${entry.session.id}-${date}-${entry.occurrence?.id ?? 'recurring'}`}>
                      <ScheduleRow entry={entry} onClick={() => onEdit(entry.session.id)} />
                      {linkedStudents.length > 0 && (
                        <div className="student-context-links schedule-student-links">
