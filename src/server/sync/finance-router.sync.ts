...
async function validatePayer(db: D1Database, workspaceId: string, type: string, id: string): Promise<void> {
  if (type === 'appointments.client') {
    const found = await db.prepare(`SELECT 1 AS found FROM appointments_clients WHERE workspace_id=?1 AND id=?2 AND active=1 AND deleted_at IS NULL`).bind(workspaceId,id).first<{found:number}>();
...
export const financeSyncHandler: ModuleSyncHandler = {
  ...
  async apply(db: D1Database, workspaceId: string, mutation: SyncMutation): Promise<void> {
    ...
    const exists = await db.prepare(`SELECT 1 AS found FROM finance_receipts WHERE workspace_id=?1 AND id=?2 LIMIT 1`).bind(workspaceId,mutation.entityId).first<{found:number}>();
    if (exists) return;
    await db.prepare(`INSERT INTO finance_receipts(
      ...
      .bind(mutation.entityId,workspaceId,parsed.payerRefType,parsed.payerRefId,parsed.amountPence,parsed.receivedAt,parsed.paymentMethod,parsed.sourceModule,parsed.sourceEntityType,parsed.sourceEntityId,parsed.note).run();
  },
