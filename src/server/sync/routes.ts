...
const sessionDetailsSchema = z.object({
  ...
  if (value.scheduleStatus === 'confirmed' && value.weekday === null) {
    ctx.addIssue({ code: 'custom', message: 'SCHEDULE_DAY_REQUIRED', path: ['weekday'] });
  }
  if (value.scheduleStatus === 'confirmed' && value.startTime === null) {
    ctx.addIssue({ code: 'custom', message: 'SCHEDULE_TIME_REQUIRED', path: ['startTime'] });
  }
});
...
async function currentSessionFinanceShape(db: D1Database, workspaceId: string, sessionId: string) {
  const session = await db.prepare(
    ...
    price_basis: 'total_session' | 'per_student';
  ...
  const students = await db.prepare(
    `SELECT student_id FROM tutoring_session_students
     WHERE workspace_id=?1 AND recurring_session_id=?2 ORDER BY student_id`,
  ).bind(workspaceId, sessionId).all<{ student_id: string }>();
  return { session, studentIds: (students.results ?? []).map((row) => row.student_id) };
}

async function applyTutoringHardeningMutation(
  db: D1Database,
  workspaceId: string,
  ...
  if (mutation.operation === 'session.details.update') {
    ...
    const hasHistory = Boolean(await db.prepare(
      `SELECT 1 AS found FROM tutoring_occurrences
       WHERE workspace_id=?1 AND recurring_session_id=?2 AND status IN ('completed','cancelled','missed') LIMIT 1`,
    ...
    if (hasHistory) {
      ...
      const financeChanged =
        parsed.priceBasis !== current.session.price_basis
    ...
    for (const studentId of parsed.studentIds) {
      const exists = await db.prepare(
        `SELECT 1 AS found FROM tutoring_students WHERE workspace_id=?1 AND id=?2 AND deleted_at IS NULL`,
    ...
    const result = await db.prepare(
      `UPDATE tutoring_recurring_sessions SET
         title=?1,session_type=?2,schedule_status=?3,weekday=?4,start_time=?5,
         duration_minutes=?6,travel_minutes=?7,location=?8,price_basis=?9,
         default_price_pence=?10,expected_student_count=?11,center_cut_bps=?12,
         updated_at=CURRENT_TIMESTAMP
       WHERE workspace_id=?13 AND id=?14 AND active=1 AND deleted_at IS NULL`,
    ).bind(
      parsed.title, parsed.sessionType, parsed.scheduleStatus,
      parsed.scheduleStatus === 'pending' ? null : parsed.weekday,
      parsed.scheduleStatus === 'pending' ? null : parsed.startTime,
      parsed.durationMinutes, parsed.travelMinutes, parsed.location, parsed.priceBasis,
      parsed.defaultPricePence, parsed.expectedStudentCount, parsed.centerCutBps,
      workspaceId, mutation.entityId,
    ).run();
    if ((result.meta?.changes ?? 0) === 0) throw new Error('SESSION_NOT_FOUND');
    await db.prepare(
      `DELETE FROM tutoring_session_students WHERE workspace_id=?1 AND recurring_session_id=?2`,
    ).bind(workspaceId, mutation.entityId).run();
    for (const studentId of parsed.studentIds) {
      await db.prepare(
        `INSERT INTO tutoring_session_students(workspace_id,recurring_session_id,student_id)
         VALUES(?1,?2,?3)`,
      ).bind(workspaceId, mutation.entityId, studentId).run();
    }
    return true;
  }
...
async function applyMutation(
  ...
  try {
    ...
    if (mutation.moduleKey === 'tutoring') {
