...
export function packageUnitShare(totalPence: number, size: number, position: number): number {
  ...
  // Difference of cumulative floors guarantees that every penny is assigned
  // exactly once and the sum across all positions equals totalPence.
...
export function packageProgress(
  ...
  if (openingCompleted > size) {
    throw new Error('Opening progress cannot exceed package size');
  }
...
export function canChangeOpeningProgress(
  ...
  // Once real lessons exist, opening progress is historical input and must not
  // be silently rewritten. Re-submitting the same value remains idempotent.
